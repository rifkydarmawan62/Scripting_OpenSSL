...
try:
    ...
    FILE_INPUT = pilih_file(title = "*Pilih file untuk dikonversi ke format .der", filetypes = [("Privacy Enhanced Mail", "*.pem"), ("Semua File", "*.*")])
    if FILE_INPUT:
        ...
        FILE_OUTPUT = f"{DIREKTORI_FOLDER}/{NAMA_FILE}.der"
        PERINTAH = f"openssl pkey -in \"{FILE_INPUT}\" -out \"{FILE_OUTPUT}\" -outform DER"
        ...
        try:
            ...
            print(f"{Fore.LIGHTRED_EX}Konversi kunci ke file .der gagal!{Fore.RESET}")
            if exists(FILE_OUTPUT):
                remove(FILE_OUTPUT)
        else:
            print(f"{Fore.LIGHTGREEN_EX}Kunci telah dikonversi ke file .der{Fore.RESET}")
    else:
